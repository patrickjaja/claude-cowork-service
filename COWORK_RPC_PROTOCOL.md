# Cowork RPC Protocol Reference - v1.15962.0
...
>
> **Re-validated against v1.15962.0 (2026-06-26): one additive RPC, no daemon change required.** Socket name (`cowork-vm-service`), wire framing (4-byte big-endian length prefix + JSON, 10 MB max), all 22 prior RPC methods, all 7 event types (`onStdout`/`onStderr`/`onExit`/`onError`/`onNetworkStatus`/`onApiReachability`/`onStartupStep`), and the full spawn param object + 34-key env set + CLI flag strings are byte-identical (modulo minified wrapper renames) to v1.15200.0. **One new RPC + one new field**, both handled by the daemon's existing tolerance: (1) **`getNetworkDrives`** - Desktop `Wwt(){const A=await bI("getNetworkDrives");return A?.drives ?? []}` (Windows VM network-drive enumeration); the daemon has no case for it, so it hits the unknown-method default (`WriteResponse(conn, req.ID, nil)` -> `{"success":true,"result":null}`), Desktop evaluates `(null)?.drives ?? []` = `[]`, and the sole caller is try/catch-wrapped -> silent no-op (semantically correct on Linux: no VM network drives). (2) **`configure.networkDrives`** field - `configureParams` decodes with plain `json.Unmarshal` (no `DisallowUnknownFields`), so the extra field is silently ignored. **No daemon change required.** Optional: add an explicit no-op `getNetworkDrives` handler returning `{drives:[]}` to silence the daemon's unknown-method debug log + the Desktop-side `warn("getNetworkDrives unavailable; keeping prior snapshot")`. Prior: re-validated against v1.15200.0 (2026-06-23) with no change; its new IPC handlers (`directMcpCallTool`/`directMcpListResources`/`directMcpReadResource`, `getCrForSession`/`getCrRefSummary`, `reportCommitHash`) are renderer<->Electron-main IPC and do NOT travel over this socket.
